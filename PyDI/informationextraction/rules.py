...
def parse_storage_gb(text: str) -> Optional[float]:
    ...
    if not text or not isinstance(text, str):
        return None
    try:
        # Use normalize_units to handle unit conversion
        normalized = normalize_units(text)
        if not normalized:
            return None
        
        # Extract numeric value and convert common units to GB
        match = re.search(r'(\d+(?:\.\d+)?)\s*(GB|TB|MB|KB|B)', normalized.upper())
        if match:
...
def parse_power_w(text: str) -> Optional[float]:
    ...
    try:
        ...
        match = re.search(r'(\d+(?:\.\d+)?)\s*(kW|W|mW|MW)', normalized.upper())
        if match:
            value, unit = match.groups()
            value = float(value)
