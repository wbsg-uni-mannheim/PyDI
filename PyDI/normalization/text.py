...
try:
    ...
    logger.warning("NLTK not available. Advanced tokenization features will be limited.")
    ...
    def __init__(
        ...
        if NLTK_AVAILABLE:
            try:
                ...
                logger.warning(f"Stop words for '{language}' not available. Disabling stop word removal.")
                self.stop_words = set()
                self.stemmer = None
        else:
            self.stemmer = None
        ...
        self.alphanumeric_split_pattern = re.compile(r'(?<=[a-zA-Z])(?=\d)|(?<=\d)(?=[a-zA-Z])')
    ...
    def process_column(
        ...
        **kwargs
        ...
        if operation == 'remove':
            ...
            extracted = series.apply(lambda x: self.extract_content(x, **kwargs))
