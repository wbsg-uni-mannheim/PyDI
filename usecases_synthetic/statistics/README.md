...
2. **splits** — combined train / val / test breakdown: per-EM-pair `total` / `positive` / `negative` / `pos_rate` plus a fusion (validation / test) entity-count block at the bottom. Variants use the regenerated EM gold copies; missing split files emit blanks.
