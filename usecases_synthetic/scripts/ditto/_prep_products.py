...
* products_1 / products_2
    train = synthetic-side products_1_2_products_2_train.csv (1831 pairs)
    val   = synthetic-side products_1_2_products_2_val.csv   (175 pairs)
    test  = synthetic-side products_1_2_products_2_test.csv  (180 pairs)

* products_1 / products_3
    train = ... 1599 pairs ; val 153 ; test 163

* products_1 / products_4
    train = ... 1326 pairs ; val 140 ; test 121

Products uses **pure PyDI throughout** (same pattern as music): there
is no ADI/`automatic-data-integration` pipeline output for products,
so the train pool is PyDI's EM gold splits directly. The 3 pairs are
mixed into a single multi-pair training set; the Ditto checkpoint at
``cache/ditto_checkpoints/products/best`` covers all 3 pairs.

Data source: the **synthetic-side** EM gold + source data, resolved
...
committee runner maps them at inference (products uses an identity
column_mapping). This is the surface wide inference serializes against, so
the baseline checkpoint trains on the same fields it scores. Empty fields
are dropped by Ditto's serializer. The matching ``train.py --domain
products`` invocation sources the same field list.
...
Outputs (all under ``usecases_synthetic/output/ditto/products/``):
    train.json.gz
    val.json.gz
    test.json.gz
...
Run from the repo root::
    ...
    python usecases_synthetic/scripts/ditto/_prep_products.py
"""
...
class Pair:
    ...
    src_left: str
    src_right: str
    pydi_stem: str  # PyDI gold filename stem


PAIRS: list[Pair] = [
    Pair(
        name="products_1_products_2",
        src_left="products_1",
        src_right="products_2",
        pydi_stem="products_1_2_products_2",
    ),
    Pair(
        name="products_1_products_3",
        src_left="products_1",
        src_right="products_3",
        pydi_stem="products_1_2_products_3",
    ),
    Pair(
        name="products_1_products_4",
        src_left="products_1",
        src_right="products_4",
        pydi_stem="products_1_2_products_4",
    ),
]
