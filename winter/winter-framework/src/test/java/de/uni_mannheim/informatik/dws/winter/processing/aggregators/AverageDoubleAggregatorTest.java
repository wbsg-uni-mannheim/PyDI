 ...
 *
 */
public class AverageDoubleAggregatorTest extends TestCase {
	
	/**
	 * Test method for {@link de.uni_mannheim.informatik.dws.winter.processing.aggregators.AverageAggregator#AverageAggregator()}.
	 */
	public void testAverageAggregator() {
		...
		Processable<Pair<Integer, Double>> result = values.aggregate(new SimpleKeyValueMapper<>((d)->0,(d)->d), new AverageDoubleAggregator<>());
		...
		assertEquals(3, average.intValue());
		...
		result = values.aggregate(new SimpleKeyValueMapper<>((d)->0,(d)->d), new AverageDoubleAggregator<>());
