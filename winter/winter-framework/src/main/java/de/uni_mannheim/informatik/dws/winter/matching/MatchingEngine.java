...
/**
 * The matching engine provides access to the matching algorithms for schema
 * matching and identity resolution.
 * 
 * @author Oliver Lehmberg (oli@dwslab.de)
 * 
 * @param <RecordType>
 *            the type of records that are matched
...
public class MatchingEngine<RecordType extends Matchable, SchemaElementType extends Matchable> {
	 ...
	 * reduce the number of internally compared {@link AbstractRecord}s the
	 * functions can be executed in a <i>symmetric</i>-mode. Here it will be
	 * assumed, that that the {@link MatchingRule} is symmetric, meaning that
	 * score(a,b) = score(b,a). Therefore the pair (b,a) can be omitted.
	 * Normally, this option can be set to <b>true</b> in most of the cases, as
	 * most of the common similarity functions (e.g.
	 * {@link LevenshteinSimilarity}, and {@link TokenizingJaccardSimilarity})
	 * are symmetric, meaning sim(a,b) = sim(b,a).
	 * 
	 * @param dataset
	 *            The data set
	 * @param rule
	...
	/**
	 * Runs the Duplicate Detection on a given {@link DataSet} with additional
	 * schema correspondences. In order to reduce the number of internally
	 * compared {@link AbstractRecord}s the functions can be executed in a
	 * <i>symmetric</i>-mode. Here it will be assumed, that that the
	 * {@link MatchingRule} is symmetric, meaning that score(a,b) = score(b,a).
	 * Therefore the pair (b,a) can be omitted. Normally, this option can be set
	 * to <b>true</b> in most of the cases, as most of the common similarity
	 * functions (e.g. {@link LevenshteinSimilarity}, and
	...
	/**
	 * Runs identity resolution on the given data sets using the provided
	 * matching rule and blocker.
	 * 
	 * @param dataset1
	 *            The first data set
	 * @param dataset2
	 ...
	 *            are then checked by the {@link MatchingRule}.
	 * @return A list of correspondences
	 */
	...
	/**
	 * 
	 * Runs identity resolution on the provided datasets. The blocking functions
	 * generate the terms which are used to calculate similarity using the
	 * vector space model.
	 * 
	 * @param dataset1
	 *            the first dataset
	...
	/**
	 * Runs schema matchign on the given data sets using the provided matching
	 * rule and blocker. The data sets must contain the attribute features as
	 * records.
	 * 
	 * @param schema1
	 *            The first data set
	 ...
	 *            (Optional) Instance correspondences between the data sets that
	 *            tell the matching rule which instance values to compare.
	 * @param rule
	 *            The {@link MatchingRule} that is used to compare the records.
	 * @param blocker
	 *            The {@link AbstractBlocker} that generates the pairs, which
	...
	/**
	 * 
	 * Runs label-based schema matching on the provided datasets using the
	...
	/**
	 * 
	 * Runs label-based schema matching on the provided dataset using the
	 * specified comparator.
	 * 
	 * @param schema
	 *            The schema
	...
	/**
	 * 
	 * Runs instance-based schema matching on the provided datasets. The
	 * blocking functions generate the terms which are used to calculate
	 * similarity using the vector space model.
	 * 
	 * @param dataset1
	 *            the first dataset
	 * @param dataset2
	...
	/**
	 * 
	 * Runs duplicate-based schema matching between the provided schemas. First,
	 * the {@link VotingMatchingRule} rule is evaluated for all provided
	 * instance correspondences to create votes. Optionally, these votes can be
	 * filtered by the voteFilter, i.e. to limit the number of votes that each
	 * value can cast. Then, the {@link CorrespondenceAggregator} voteAggregator
	 * combines the votes into final correspondences.
	 * 
	 * @param schema1
	 *            The first schema
	 * @param schema2
	 ...
	 * @param schemaBlocker
	 *            The blocker that creates potential pairs of attributes. Use
	 *            {@link NoSchemaBlocker} is all combinations should be
	 *            considered.
	...
	/**
	 * 
	 * Returns the k correspondences for each record on the left-hand side with
	 * the highest similarity score
	 * 
	 * @param correspondences
	 *            The correspondences that should be filtered
	 * @param k
	 *            The desired number of correspondences for each record on the
	 *            left-hand side
	 * @param similarityThreshold
	 *            The minimum similarity for a correspondence to appear in the
	 *            result
	...
	/**
	 * 
	 * Returns the k correspondences for each attribute on the left-hand side
	 * with the highest similarity score
	 * 
	 * @param correspondences
	 *            The correspondences that should be filtered
	 * @param k
	 *            The desired number of correspondences for each attribute on
	 *            the left-hand side
	 * @param similarityThreshold
	 *            The minimum similarity for a correspondence to appear in the
	 *            result
